...
$query = "SELECT * from zon_games WHERE game_category='$category' order by id asc limit $row,$limit";
$run = mysqli_query($con, $query);
if (num_rows(T_ZON_GAMES, "game_category='$category'") > 0) {
    while ($game = mysqli_fetch_assoc($run)) {
        if ($zon['config']['theme'] === 'garud') { ?>
            <div class="game wow <?php echo $zon['config']['animate_class'] ?> ">
    ...
    echo "There are no games in this category right now.";
} ?>